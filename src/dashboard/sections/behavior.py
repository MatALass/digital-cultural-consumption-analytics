...
def render(df: pd.DataFrame, baseline_df: pd.DataFrame) -> None:  # noqa: ARG001
    ...
    with c1:
        st.plotly_chart(
            bar_count(
                df, "access_style", "Access style mix", color_map=PALETTE_ACCESS_STYLE
            ),
            width="stretch",
        )
    with c2:
        st.plotly_chart(
            bar_count(
                df,
                "legality_type",
                "Legal vs illegal self-positioning",
                orientation="h",
            ),
            width="stretch",
        )
    ...
    with c3:
        ...
        st.plotly_chart(
            radar_chart(
                ...
                "Average risk pattern by segment",
            ),
            width="stretch",
        )
    with c4:
        st.plotly_chart(
            violin(
                ...
                "generation",
                "risk_behavior_score",
                "Risk score distribution by generation",
    ...
    with c5:
        st.plotly_chart(
            bar_metric_by_group(
                ...
                "generation",
                "vpn_score",
                "Average VPN score by generation",
                agg="mean",
                color_map=PALETTE_GENERATION,
            ),
            width="stretch",
    ...
    with c6:
        st.plotly_chart(
            scatter(
                ...
                "access_style",
                "Engagement vs raw risk intensity",
                color_map=PALETTE_ACCESS_STYLE,
                size="raw_illicit_channel_count",
            ),
            width="stretch",
        )
    ...
    with c7:
        st.plotly_chart(
            bar_metric_by_group(
                ...
                "generation",
                "dns_score",
                "Average DNS-tinkering score by generation",
                agg="mean",
                color_map=PALETTE_GENERATION,
            ),
            width="stretch",
    ...
    with c8:
        st.plotly_chart(
            bar_metric_by_group(
                ...
                "generation",
                "cracked_apps_score",
                "Average cracked-app score by generation",
                agg="mean",
                color_map=PALETTE_GENERATION,
            ),
            width="stretch",
