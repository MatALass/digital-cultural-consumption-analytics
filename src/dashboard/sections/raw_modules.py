...
def render(df: pd.DataFrame, baseline_df: pd.DataFrame) -> None:  # noqa: ARG001
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(violin(df, 'consumer_segment', 'raw_device_diversity', 'Device diversity by segment', color_map=PALETTE_SEGMENT), width='stretch')
    with c2:
        st.plotly_chart(bar_metric_by_group(df, 'generation', 'raw_illicit_method_count', 'Average illicit-method breadth by generation', agg='mean'), width='stretch')

    c3, c4 = st.columns(2)
    with c3:
        st.plotly_chart(heatmap_share(df, 'consumer_segment', 'payment_bucket', 'Payment orientation within segments'), width='stretch')
    with c4:
        st.plotly_chart(box(df, 'payment_bucket', 'legal_value_perception_score', 'Legal value perception by payment bucket'), width='stretch')
