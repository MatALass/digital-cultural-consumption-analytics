...
def render(df: pd.DataFrame, baseline_df: pd.DataFrame) -> None:  # noqa: ARG001
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(violin(df, 'generation', 'monthly_spending', 'Spending distribution by generation', color_map=PALETTE_GENERATION), width='stretch')
    with c2:
        sunburst_df = (
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.plotly_chart(bar_metric_by_group(df, 'urbanity_bucket', 'raw_content_category_count', 'Average content breadth by urbanity', agg='mean', orientation='h', sort='desc'), width='stretch')
    with c4:
    ...
    deep_dive = spending_distribution_by_group(df, 'generation').rename(columns={'payer_rate': 'paying_rate'})
    deep_dive['share_of_filtered'] = (deep_dive['respondents'] / max(len(df), 1)).round(3)
    deep_dive['share_of_full_population'] = deep_dive['generation'].map(
