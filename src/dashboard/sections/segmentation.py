...
from dashboard.utils.metrics import opportunity_segments, segment_summary
...
def render(df: pd.DataFrame, baseline_df: pd.DataFrame) -> None:  # noqa: ARG001
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(bar_count(df, 'consumer_segment', 'Consumer segment distribution', orientation='h', color_map=PALETTE_SEGMENT), width='stretch')
    with c2:
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.plotly_chart(bar_grouped(grouped, 'consumer_segment', 'value', 'metric', 'Average score profile by segment'), width='stretch')
    with c4:
        st.plotly_chart(
    ...
    st.dataframe(segment_summary(df), width='stretch', hide_index=True)

    st.subheader('Priority segments')
    priority = opportunity_segments(df).head(6).copy()
    if not priority.empty:
        priority['conversion_headroom'] = (priority['conversion_headroom'] * 100).round(1)
        priority['risk_rate'] = (priority['risk_rate'] * 100).round(1)
