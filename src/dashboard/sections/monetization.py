...
def render(df: pd.DataFrame, baseline_df: pd.DataFrame) -> None:  # noqa: ARG001
    ...
    with c1:
        st.plotly_chart(
            funnel_chart(funnel_df, "stage", "value", "Conversion funnel"),
            width="stretch",
        )
    with c2:
        tree_df = (
            df.groupby(["consumer_segment", "value_tier"], dropna=False)
            .size()
        ...
        st.plotly_chart(
            treemap(
                tree_df,
                path=["consumer_segment", "value_tier"],
                values="respondents",
                title="Segment × value tier mix",
    ...
    with c4:
        st.plotly_chart(
            scatter(
                ...
                "monthly_spending",
                "consumer_segment",
                "Engagement vs monthly spending",
                color_map=PALETTE_SEGMENT,
    ...
    st.plotly_chart(
        bar_grouped(
            ...
            "share",
            "access_style",
            "Access style share by value tier",
    ...
    st.dataframe(
        percentile_table(df, "consumer_segment", "monthly_spending"),
        width="stretch",
        hide_index=True,
    )
