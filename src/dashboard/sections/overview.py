...
def render(df: pd.DataFrame, baseline_df: pd.DataFrame) -> None:
    ...
    st.caption(
        "Decision-oriented dashboard built from the curated analytical layer and raw module enrichment."
    )
    ...
    row1 = st.columns(6)
    respondent_delta = (
        f"{len(df) - len(baseline_df):,} vs full" if is_filtered else None
    )
    ...
    row2[2].metric(
        "Under-monetized engaged", f"{k['pct_under_monetized_engaged'] * 100:.1f}%"
    )
    ...
    with c1:
        st.plotly_chart(
            histogram(
                df, "monthly_spending", "Monthly spending distribution", nbins=35
            ),
            width="stretch",
        )
    with c2:
        st.plotly_chart(
            bar_metric_by_group(
                ...
                "Average content breadth by generation",
                agg="mean",
    ...
    with c3:
        st.plotly_chart(
            scatter(
                ...
                title="Engagement vs monthly spending",
    ...
    with c4:
        ...
        st.plotly_chart(
            line_chart(
                ...
                "spending_quintile",
                "avg_engagement",
                "Average engagement by spending quintile",
                markers=True,
            ),
            width="stretch",
        )
    ...
    with b2:
        ...
        if not opportunities.empty:
            opportunities["conversion_headroom"] = (
                opportunities["conversion_headroom"] * 100
            ).round(1)
    ...
    st.info(
        f"The top quartile starts at {k['p75_spending']:.1f}€, which confirms revenue concentration in a limited share of respondents."
    )
    st.warning(
        f"{k['pct_under_monetized_engaged'] * 100:.1f}% of the filtered population appears engaged but still non-paying, which is the cleanest conversion opportunity."
    )
    st.success(
        f"Raw module enrichment adds an average of {k['avg_raw_content_depth']:.2f} observed content-category signals per respondent."
    )
