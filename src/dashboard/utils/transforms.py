...
def _consumer_segment(row: pd.Series) -> str:
    if row["high_value_user"] and row["access_style"] == "Legal Paid" and row["risk_behavior_score"] <= 1:
        return "Premium Legal Users"
    if row["high_value_user"] and row["raw_illicit_channel_count"] >= 1:
        return "Hybrid Monetizers"
    if row["monthly_spending"] <= 0 and row["engagement_score"] >= 0.65 and row["raw_content_category_count"] >= 3:
...
def _build_raw_feature_layer(raw_df: pd.DataFrame, target_len: int) -> pd.DataFrame:
    ...
    module_cols = {name: _select_columns(raw, pattern) for name, pattern in RAW_MODULE_PATTERNS.items()}
    ...
    out["raw_content_paid_score"] = _rowwise_numeric_mean(raw[module_cols["qbu1_paid_matrix"]])
    out["raw_content_category_count"] = _rowwise_non_null_count(raw[module_cols["qbu1_paid_matrix"]])
    out["raw_device_diversity"] = _rowwise_selected_count(raw[module_cols["qbu5a_devices"]])
    ...
    out["raw_illicit_frequency_mean"] = _rowwise_numeric_mean(raw[module_cols["qbu11_illegal_frequency"]])
    out["raw_illicit_method_count"] = _rowwise_selected_count(raw[module_cols["qbu11b_illicit_methods"]])
    out["raw_stream_ripping_score"] = _rowwise_numeric_mean(raw[module_cols["qbu12_stream_ripping"]])
    out["raw_legal_offer_advantage_count"] = _rowwise_selected_count(raw[module_cols["qbol3_legal_advantages"]])
    out["raw_legal_motivation_count"] = _rowwise_selected_count(raw[module_cols["qbol4_legal_motivations"]])
    out["raw_behavioral_coverage_rate"] = module_df.notna().mean(axis=1) if not module_df.empty else 0.0
    return out.reset_index(drop=True)

def build_analytics_dataset(processed_df: pd.DataFrame, raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["monthly_spending"] = pd.to_numeric(out["monthly_spending"], errors="coerce").fillna(0)
    out["sex"] = out["sex"].map(_normalize_sex)

    out["age_group"] = pd.cut(
        out["age"], bins=AGE_GROUP_BINS, labels=AGE_GROUP_LABELS, include_lowest=True, right=True
    ).astype(str)
    ...
    out["internet_intensity_score"] = out["internet_frequency"].map(INTERNET_FREQUENCY_SCORE).fillna(0)
    out["cultural_frequency_score"] = out["cultural_frequency"].map(CULTURAL_FREQUENCY_SCORE).fillna(0)
    ...
    out["paid_access_score"] = out["paid_services_access"].map(PAID_ACCESS_SCORE).fillna(0)
    out["vpn_score"] = out["vpn_usage"].map(VPN_SCORE).fillna(0)
    out["dns_score"] = out["dns_settings_usage"].map(DNS_SCORE).fillna(0)
    out["download_risk_score"] = out["streaming_download_type"].map(DOWNLOAD_RISK_SCORE).fillna(0)
    out["cracked_apps_score"] = out["cracked_apps_usage"].map({"Non": 0, "Oui": 3}).fillna(0)
    ...
    out["risk_behavior_score"] = (
        out["vpn_score"] + out["dns_score"] + out["download_risk_score"] + out["cracked_apps_score"]
    )
    ...
    out["content_depth_score"] = (
        0.45 * (out["raw_content_category_count"] / max(float(out["raw_content_category_count"].max()), 1.0))
        + 0.25 * (out["raw_device_diversity"] / max(float(out["raw_device_diversity"].max()), 1.0))
        + 0.30 * out["raw_behavioral_coverage_rate"]
    )
    out["raw_risk_intensity_score"] = (
        out["raw_illicit_channel_count"] + out["raw_illicit_method_count"] + out["raw_stream_ripping_score"]
    )
    ...
    out["hybrid_access_user"] = out["legality_type"].astype(str).isin(
        ["Autant", "Souvent légale", "Souvent illégale"]
    )
    out["risky_behavior_user"] = (out["risk_behavior_score"] >= 4) | (out["raw_risk_intensity_score"] >= 3)
    ...
    out["value_tier"] = out["monthly_spending"].apply(lambda x: _value_tier(float(x), q50, q75, q90))
