...
from dashboard.config import DATAMAP_PATH, PROCESSED_COLUMN_RENAMES, PROCESSED_DATA_PATH, RAW_BAROMETER_PATH
...
def load_processed_data() -> pd.DataFrame:
    ...
    return df.rename(columns=PROCESSED_COLUMN_RENAMES).drop(columns=['Unnamed: 0'], errors='ignore')
...
@st.cache_data(show_spinner=False)
def build_cached_analytics(processed: pd.DataFrame, raw_barometer: pd.DataFrame) -> pd.DataFrame:
