...
def compute_overview_kpis(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "legal_paid_rate": float(
            df["access_style"].astype(str).eq("Legal Paid").mean()
        ),
    }
...
def engagement_vs_spending_quintiles(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame(
            columns=["spending_quintile", "avg_engagement", "avg_risk", "respondents"]
        )
    working = df[["monthly_spending", "engagement_score", "risk_behavior_score"]].copy()
    working["spending_quintile"] = pd.qcut(
        working["monthly_spending"].rank(method="first"),
        5,
    ...
    return (
        ...
        .agg(
            avg_engagement=("engagement_score", "mean"),
            avg_risk=("risk_behavior_score", "mean"),
            respondents=("spending_quintile", "size"),
        )
        .reset_index()
    )
...
def access_mix_summary(df: pd.DataFrame, group_col: str) -> pd.DataFrame:
    ...
    ctab = pd.crosstab(
        df[group_col].astype(str), df["access_style"].astype(str), normalize="index"
    )
    return ctab.reset_index().melt(
        id_vars=group_col, var_name="access_style", value_name="share"
    )
...
def compare_kpis_to_baseline(
    filtered_df: pd.DataFrame, baseline_df: pd.DataFrame
) -> pd.DataFrame:
    metric_map = {
        "median_spending": "Median spending (€)",
        "pct_paying": "Paying rate",
        "pct_high_value": "High-value rate",
        "pct_risky": "Risky behavior rate",
        "pct_under_monetized_engaged": "Under-monetized engaged rate",
        "avg_engagement": "Average engagement score",
        "avg_monetization": "Average monetization score",
    }
    filtered = compute_overview_kpis(filtered_df)
    baseline = compute_overview_kpis(baseline_df)
    rows: list[dict[str, float | str]] = []
    for key, label in metric_map.items():
        baseline_value = float(baseline.get(key, 0.0))
        filtered_value = float(filtered.get(key, 0.0))
        rows.append(
            {
                "metric": label,
                "filtered_value": filtered_value,
                "baseline_value": baseline_value,
                "absolute_delta": filtered_value - baseline_value,
                "relative_delta": 0.0
                if baseline_value == 0
                else (filtered_value - baseline_value) / baseline_value,
            }
        )
    return pd.DataFrame(rows)


def opportunity_segments(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame(
            columns=[
                "consumer_segment",
    ...
    grouped = (
        df.assign(
            engaged_non_payer=(df["engagement_score"].ge(0.65) & ~df["paying_user"])
        )
        .groupby("consumer_segment", dropna=False)
        .agg(
            respondents=("consumer_segment", "size"),
            engaged_non_payers=("engaged_non_payer", "sum"),
            conversion_headroom=("under_monetized_engaged_user", "mean"),
            risk_rate=("risky_behavior_user", "mean"),
        )
        .reset_index()
    )
    return grouped.sort_values(
        ["engaged_non_payers", "conversion_headroom", "respondents"],
