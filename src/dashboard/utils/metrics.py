...
def compare_kpis_to_baseline(filtered_df: pd.DataFrame, baseline_df: pd.DataFrame) -> pd.DataFrame:
    metric_map = {
        ...
        rows.append(
            {
                ...
                'relative_delta': 0.0 if baseline_value == 0 else (filtered_value - baseline_value) / baseline_value,
            }
        )
...
def opportunity_segments(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame(columns=['consumer_segment', 'respondents', 'engaged_non_payers', 'conversion_headroom', 'risk_rate'])
    grouped = (
        df.assign(engaged_non_payer=(df['engagement_score'].ge(0.65) & ~df['paying_user']))
    ...
    return grouped.sort_values(['engaged_non_payers', 'conversion_headroom', 'respondents'], ascending=[False, False, False])
