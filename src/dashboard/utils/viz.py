...
def bar_count(
    ...
    *,
    ...
    if orientation == "h":
        ...
        fig = px.bar(
            plot_df, x=column, y="value", color=column, color_discrete_map=color_map
        )
    if normalize:
        fig.update_yaxes(tickformat=".0%") if orientation == "v" else fig.update_xaxes(
            tickformat=".0%"
        )
...
def heatmap_share(df: pd.DataFrame, index: str, columns: str, title: str):
    if df.empty:
        return _empty_figure(title)
    ctab = pd.crosstab(
        df[index].astype(str), df[columns].astype(str), normalize="index"
    )
    fig = px.imshow(
        ctab, aspect="auto", text_auto=".0%", color_continuous_scale="Tealgrn"
    )
...
def scatter(
    ...
    *,
    ...
    if df.empty:
        return _empty_figure(title)
    fig = px.scatter(
        df, x=x, y=y, color=color, opacity=0.7, color_discrete_map=color_map, size=size
    )
    return _apply_layout(fig, title)
...
def treemap(
    ...
    *,
    ...
    if df.empty:
        return _empty_figure(title)
    fig = px.treemap(
        df, path=path, values=values, color=color, color_discrete_map=color_map
    )
    return _apply_layout(fig, title)
...
def bar_grouped(
    ...
    *,
    ...
    if df.empty:
        return _empty_figure(title)
    fig = px.bar(
        df, x=x, y=y, color=color, barmode="group", color_discrete_map=color_map
    )
    return _apply_layout(fig, title)


def radar_chart(
    df: pd.DataFrame, category_col: str, value_cols: Iterable[str], title: str
):
    if df.empty:
        return _empty_figure(title)
    ...
    for _, row in df.iterrows():
        ...
        fig.add_trace(
            go.Scatterpolar(
                r=values, theta=theta, fill="toself", name=str(row[category_col])
            )
        )
    fig.update_layout(
        polar={
            "radialaxis": {
                "visible": True,
                "range": [0, max(1.0, math.ceil(df[value_cols].max().max()))],
            }
        }
    )
    return _apply_layout(fig, title)
